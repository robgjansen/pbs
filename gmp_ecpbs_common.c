...
void gmp_ecpbs_Fhash(epoint *result, char *info, gmp_ecpbs_parameters *pbs) {
	big hash = mirvar(0);
	gmp_ecpbs_Hhash(hash, info, pbs->q);
...
void gmp_ecpbs_hash_epsilon(big result, epoint *alpha, epoint *beta, epoint *z,
	...
	gmp_ecpbs_Hhash(result, buffer, parameters->q);
	free(buffer);
}
...
int gmp_ecpbs_verify(gmp_ecpbs_signature *signature, char *info, char *message,
	...
	gmp_ecpbs_mod(workspace->result, parameters->q);

	gmp_ecpbs_Fhash(workspace->z, info, parameters);
	...
	gmp_ecpbs_hash_epsilon(workspace->check, workspace->alpha, workspace->beta,
